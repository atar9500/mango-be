...
const getNotesService = (author: string): NotesService => ({
  createNote: async args => await createNote(s3, db, {author, ...args}),
  deleteNote: async args => await deleteNote(s3, db, {author, ...args}),
  editNote: async args => await editNote(s3, db, {author, ...args}),
  getNotes: async args => await getNotes(db, {author, ...args}),
  getNoteContent: async args => await getNoteContent(s3, {author, ...args}),
});
